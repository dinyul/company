(function() {
	$(".proCitySelAll").click(function(event) {
		...
		$(".provinceCityAll").css("bottom", "0px").css("left","0px").toggle();
		...
		$(".provinceCityAll").find(".tabs").find("a").click(function() {
			...
			if ($(this).attr("tb") == "countyAll" && $(".cityAll .list .current").val() == null && $(".provinceAll .list .current").val() == null) {
				return;
			};
			$(".provinceCityAll").find(".tabs").find("a").removeClass("current");
...
(function() {
	var isopera = new RegExp("opera").test(navigator.userAgent.toLowerCase());
	var clkIndex;
	var currentClass;
	var allCitys;
	var allProvinces;
	...
	$("body").delegate(".proCityQuery,.proCityQueryAll", (isopera ? "keypress": "keyup"),
	function(event) {
		...
		for (i = 0; i < allCitys.cities.length; i++) {
			if (v.toUpperCase() === allCitys.cities[i].cityShortPY.substring(0, v.length)) {
				htmlArr[htmlArr.length] = "<li><a style='font-size:16px;' class='allcityClass' href='javascript:' provinceId=" + allCitys.cities[i].provinceId + " cityId=" + allCitys.cities[i].id + ">" + allCitys.cities[i].name + " (<span style='color:red'>" + v.toUpperCase() + "</span>" + allCitys.cities[i].cityShortPY.substring(v.length, allCitys.cities[i].cityShortPY.length) + ")</a></li>";
				if (htmlArr.length > 9) {
					break;
					return false;
				}
			...
			if (v === allCitys.cities[i].name.substring(0, v.length)) {
				htmlArr[htmlArr.length] = "<li><a style='font-size:16px;' class='allcityClass' href='javascript:' provinceId=" + allCitys.cities[i].provinceId + " cityId=" + allCitys.cities[i].id + "><span style='color:red'>" + v + "</span>" + allCitys.cities[i].name.substring(v.length, allCitys.cities[i].name.length) + " (" + allCitys.cities[i].cityShortPY + ")</a></li>";
				if (htmlArr.length > 9) {
					break;
					return false;
				}
			...
			if (v.toLowerCase() === allCitys.cities[i].cityPinyin.substring(0, v.length)) {
				htmlArr[htmlArr.length] = "<li><a style='font-size:16px;' class='allcityClass' href='javascript:' provinceId=" + allCitys.cities[i].provinceId + " cityId=" + allCitys.cities[i].id + ">" + allCitys.cities[i].name + " (<span style='color:red'>" + v.toLowerCase() + "</span>" + allCitys.cities[i].cityPinyin.substring(v.length, allCitys.cities[i].cityPinyin.length) + ")</a></li>"
				if (htmlArr.length > 9) {
					break;
					return false;
				}
...
function sendCitiesAjax() {
	$.ajax({
		type: "get",
		url: rootPath+'/js/choAdd/queryAllCities.js',
		async: false,
		dataType: "json",
...
function viewProvince(page) {
	...
	for (var i = start; i < end; i++) {
		...
		var li = $('<li><a style="font-size:16px;" style="background: none repeat scroll 0% 0% transparent; border: 0px none;" href="javascript:onclick=viewCities(' + i + ');" id="' + p_id + '">' + p_name + '</a></li>');
		$(".province .list ul").append(li);
		$(".province .list ul li a").css("font-size","16px");
	}
...
function sendAllProvinceAjax() {
			allProvinces = allProvincesQuery.provinces;
			$("body").data("allProvinces", allProvinces);
			viewAllProvince(1);

}
function sendAllCitiesAjax() {
			allCities = allCitiesQuery.cities;
			$("body").data("CitysAll", allCitiesQuery);
			viewAllHotCities();
...
function viewAllHotCities() {
	...
	function(i, city) {
		if (city.hotCity) {
			$(".provinceAll .list ul").append("<li><a><input type='button' style='background:none;border:0px;cursor: pointer;' onclick=hotCityAddrInputAll(\'" + city.provinceId + "," + city.id + "," + city.name + "\') id='" + city.id + "' value='" + city.name + "'></a></li>");
		}
	});
}
...
function viewAllCities(i) {
	...
	//给省赋值
	$("#provinceName").val(allProvinces[i].provinceName);
	$("#provinceName_1").val(allProvinces[i].provinceName);
	allCitys = [];
	var j = 0;
	$.each(allCities,
	function(i, city) {
...
function viewAllCounties(i) {
	...
	//给市赋值
	$("#cityName").val(allCitys[i].name);
	$("#provinceName_1").val($("#provinceName_1").val()+allCitys[i].name);
	//影藏
	$(".provinceCityAll").hide();
...
function addrInputAll(i) {
	...
	$(".con .provinceAll .list a input").removeClass("current");
	$(".con .provinceAll .list a input[id='" + cityIdAll + "']").addClass("current");
