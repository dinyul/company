	...
	(function(api,util,ajax){
		...
		function initOrder(orderId) {
			ajax.post(api.baseServerUrl + "/order/orderDetail",{
				...
				if(data.serviceStatus!=1){
					window.location.href="./zhifuchenggong.html?orderId="+orderId+"&zhifutype="+orderType+"&giftKey="+giftKey;
				}
				...
				if(price!=undefined && price!=null && price!='' && (($('.pay span').html()).split('¥')[1])!=Number(price)){
					alert('商品特价活动已结束，可以继续按原价购买哦')
				}
			})
		}
		...
		function completePay(url,orderCode,flag){
			ajax.post( api.baseServerUrl+ url,{
				...
				setCookie('reFresh','')
				window.location.href="./zhifuchenggong.html?orderId="+orderCode+"&zhifutype="+orderType+"&giftKey="+giftKey;
			},function(){
				setCookie('reFresh','')
