...
var CartItem = function() {
    this.GoodsId = 0; //商品id
    this.GoodsPrice = 0; //商品原价
    this.GoodsPriceNew = 0; //商品现价
    this.GoodsCount = 0; //购买数量
    this.RepertoryCount = 0; //库存量
	this.ChildrenGoodsIds =0; //子商品
    //2016年3月1日11:56:24注释
    //this.GoodsName = "";//商品名字
...
var CartHelper = function() {
    var name = $.cookie('orgId') + 'goolaNew';
    //	alert(name);
    ...
    this.Add = function(goodsId, goodsPrice, goodsPriceNew, goodsCount, repertoryCount, childrenGoodsIds) {
        ...
        cart.Count++;
        //index 为此商品在购物车项中的下标
        var index = this.Find(goodsId,childrenGoodsIds);
        // 如果ID已存在，覆盖数量
        if (index > -1) { //说明该商品已经存在于购物车
            ...
            item.ChildrenGoodsIds = childrenGoodsIds;
            //2016年3月1日12:01:46注释
    ...
    // 改变数量
    this.Change = function(goodsId, goodsCount,childrenGoodsIds) {
    ...
    // 移出购物车
    this.Del = function(goodsId,childrenGoodsIds) {
        //从cookie中取出购物车
        var cart = this.Read();
        //检查此商品是否存在于购物车
        var index = this.Find(goodsId,childrenGoodsIds);
    ...
    // 根据ID查找
    this.Find = function(goodsId,childrenGoodsIds) {
        //将购物车从cookie中读取
        var cart = this.Read();
        ...
        for (var i = 0; i < cart.Items.length; i++) {
        	...
        	var ChildrenGoodsIds=cart.Items[i].ChildrenGoodsIds.split(',').sort(function(a,b){return a-b;})
        	var childrenGoodsIds
        	
        	if(childrenGoodsIds!=undefined){
        		childrenGoodsIds=childrenGoodsIds.split(',').sort(function(a,b){return a-b;})
        		childrenGoodsIds=childrenGoodsIds.join(',')
        	}
        	ChildrenGoodsIds=ChildrenGoodsIds.join(',')
        	
        	if ((cart.Items[i].GoodsId == goodsId  && ChildrenGoodsIds==childrenGoodsIds) || (cart.Items[i].GoodsId == goodsId && childrenGoodsIds==undefined)) {
                //如果有则将 将其下标付给 index
               	index = i;
    ...
    this.ItemToString = function(item) {
        ...
        return item.GoodsId + "||" + item.GoodsPrice + "||" + item.GoodsPriceNew + "||" + item.GoodsCount + "||" + item.RepertoryCount+ "||" + item.ChildrenGoodsIds;
    };
    ...
    this.ItemToObject = function(str) {
        ...
        item.GoodsCount = arr[3];
        item.RepertoryCount = arr[4];
		item.ChildrenGoodsIds = arr[5]
        //item.GoodsId = arr[0];
