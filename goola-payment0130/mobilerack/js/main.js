 ...
 * 加入购物车
 */
function addProductinCart(goodsId,goodsPrice,goodsPriceNew, goodsCount,count){
	if(is_close != ""){
		popShow();
		return;
	}
	...
	carHelper.Add(goodsId,goodsPrice,goodsPriceNew, goodsCount,count);
	showCartStatus();
	return true;
//	addProduct(event);
}
...
function addProduct(event) {
	var offset = $('#end').offset(), flyer = $('<img class="u-flyer del" src="images/profile-80_1.jpg"/>');
	flyer.fly({
		start: {
			left: event.pageX,
...
function showCartStatus(){
	...
	if(car.Count > 0){
		...
		//$('.cexuanf').css('padding-right','10px');
		$("#selectedTotal").html('<span id="goodsCount">0</span>');
		$(".selectedTotal1").html('<span id="goodsCount">0</span>');
	}
}
