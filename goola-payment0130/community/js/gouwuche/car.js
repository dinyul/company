...
var CartHelper = function() {
	var name = $.cookie('orgId')+'goolaNewcommunity';
//	alert(name);
	...
	this.Add = function(goodsId, goodsPrice, goodsPriceNew, goodsCount, repertoryCount) {
...
//		console.log(goodsId+"   "+ goodsPrice+"   "+ goodsPriceNew+"   "+goodsCount+"   "+repertoryCount);
	...
	// 改变数量
	this.Change = function(goodsId, goodsCount) {
		//从cookie 中读取了购物车
		var cart = this.Read();
...
//		return item.GoodsId + "||" + escape(item.GoodsName) + "||"+ escape(item.GoodsPhoto) + "||" + item.GoodsPrice + "||"+item.GoodsPriceNew + "||" + item.GoodsCount + "||" + item.ProviderId + "||" +item.RepertoryCount + "||" +item.CategoryId;
		return item.GoodsId + "||" + item.GoodsPrice + "||"+item.GoodsPriceNew + "||" + item.GoodsCount + "||" +item.RepertoryCount;
	};
